...
<div class="divTableCell"><form action="<?=base_url()?>profil/bildirimler" method="post">                
                             <input type="image"  src="<?=base_url()?>uploads/bell2.png" ><div id="spanBil"></div></form> 
                        </div>
</div>
</div>
</div>
            ...
            <button type="button" class="button special small" data-toggle="modal" data-target="#myyyyModal">Resim Ekle</button>

            <!-- Modal -->
              <div id="myyyyModal" class="modal fade" role="dialog">
                <div class="modal-dialog">
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                  <form onsubmit="return validateForm1()" id="myForm1" action="<?=base_url()?>profil/galeriresim_upload" method="post" enctype="multipart/form-data">
                    <font color="#CC3333" >RESMİ SEÇİNİZ:<br></font>
                    <input type="file" name="dosyaadi"  id="dosyaadi"  > <br>
        ...
        <section>
          ...
          ?>
            ...
            <article>
              ...
              <a data-toggle="modal" data-target="#yorumModal<?=$rs->Id?>"><img class="image" src="<?=base_url()?>uploads/<?=$rs->img;?>" alt=""  width="400" height="400"/></a>
              <div class="dropdown" style="float:right;">
                    <button class="button special small"><li class="glyphicon glyphicon-cog"></li></button>
                    <div class="dropdown-content">          
                    <a href="<?=base_url()?>profil/fotosil/<?=$rs->Id?>" onclick="return confirm('Silmek istediğinizden emin misiniz?');"><font size="2" ><i class="glyphicon glyphicon-trash"></i>&emsp;Sil</a></font>                             
              </div>                        
              </div>
              <div class="dropdown" >
                 <button class="button special small" data-toggle="modal" data-target="#yorumModal<?=$rs->Id?>" ><font size="1" >Yorumlar</font></button>

                    <div class="modal fade" id="yorumModal<?=$rs->Id?>" role="dialog">
                      <div class="modal-dialog">
                        ...
                        <div class="modal-content">
                          <div class="modal-header">
                            ...
                            <form  action="<?=base_url()?>profil/galeriyorumkaydet/<?=$rs->Id?>" method="post">                           
                            <font color="black"><textarea onclick="yorumyolla(<?=$rs->Id?>);" id="enter<?=$rs->Id?>" rows="1" name="yorum" placeholder="Yorum Yap.." ></textarea></font><br>
                            <input style="display: none;" id="yorumyolla<?=$rs->Id?>" type="submit" class="button special small" name="Gönder">
                          </form> 
                          ...
                          <?php foreach ($galeriyorumlar as $row) {
                           ...
                           ?>
                              ...
                              <li>          
                                 <div style="background-color: #F5F6F7; width: 550px; height:60px; ">
                                     ...
                                     <img src="<?=base_url()?>uploads/<?=$row->yapanimg?>">

                                     </div>
                                     &emsp;<font color="black" size="2">

                                     <?php if($row->uye_ID != $this->session->logged_in->Id) { ?>
                                       <a href="<?=base_url()?>profil/ybnprofil/<?=$row->uye_ID?>"><?=$row->uye_adsoy?></a>
                                     <?php }else { ?>
                                      <a href="<?=base_url()?>profil"><?=$row->uye_adsoy?></a>
                                     <?php } ?>



                                     <br>&emsp;<font color="black" size="2"><?=$row->yorum?></font><font size="1"><div style="float: right;"><?=$row->tarih?>  <?php 
                                       if(($row->uye_ID==$this->session->logged_in->Id) || ($row->kime_Id==$this->session->logged_in->Id) ) { ?>
                                                
                                                <a href="<?=base_url()?>profil/galeriyorumsil1/<?=$row->Id?>" onclick="return confirm('Yorumu silmek istediğinizden emin misiniz?');" class="paylasimyorumsil"><i class="glyphicon glyphicon-trash"></i></a>
                                        <?php } ?></div></font></font>
                                 </div>            
                              </li>
