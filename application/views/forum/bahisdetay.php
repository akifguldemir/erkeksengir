					...
					<div id="main">
						<div class="inner">
										    ...
										    <div class="modal-content">
										      ...
										      <div class="modal-body">
													...
													<?php foreach ($davet as $rs) { ?>
											  			...
											  			<a href="<?=base_url()?>forum/bahisdavet/<?=$fbahis[0]->Id?>/<?=$rs->kimiId?>" onclick="return confirm('Davet göndermek istediğinizden emin misiniz?');"><li><?=$rs->kimiad?>&emsp;</li></a>
							...
							<?php
								...
								foreach((array)$fbahisyorum as $rs)
								{
						...
						</div><font color="#F56A6A"><p align="right"><a href="<?=base_url()?>forum/bahispaylasimsil/<?=$rs->Id?>/<?=$bahisdonId?>" onclick="return confirm('Silmek İstediğinizden Eminmisiniz?');">

											<?php 
												if($this->session->logged_in)
														{
															if($this->session->logged_in->Id==$rs->uye_ID) { ?>
											<form action="<?=base_url()?>forum/bahispaylasimsil/<?=$rs->Id?>" method="post">
                            ...
                            <div class="panel-group" id="accordion">
                                <div class="panel panel-default">
                                    ...
                                    <div id="collapseOne" class="panel-collapse collapse" style="height: 0px;">
                                        <div class="panel-body">
                             <?php
								foreach($fbahis as $rs)
								{								
								?>  <form onsubmit="return validateForm()" id="myForm" action="<?=base_url()?>forum/bahisyorumkaydet/<?=$rs->Id?>" method="POST"> <?php } ?>
								...
								foreach($fbahis as $rs)
								{								
								?> <form action="<?=base_url()?>forum/bahisyorumresimkaydet/<?=$rs->Id?>" method="post" enctype="multipart/form-data"><?php } ?>
