					...
					<div id="main">
						<div class="inner">
										    ...
										    <div class="modal-content">
										      ...
										      <div class="modal-body">
													...
													<?php foreach ($davet as $rs) { ?>
											  			...
											  			<a href="<?=base_url()?>forum/oyundavet/<?=$foyun[0]->Id?>/<?=$rs->kimiId?>" onclick="return confirm('Davet göndermek istediğinizden emin misiniz?');"><li><?=$rs->kimiad?>&emsp;</li></a>
							...
							<?php
								...
								foreach((array)$foyunyorum as $rs)
								{
						...
						</div><font color="#F56A6A"><p align="right"><a href="<?=base_url()?>forum/oyunpaylasimsil/<?=$rs->Id?>/<?=$oyundonId?>" onclick="return confirm('Silmek İstediğinizden Eminmisiniz?');">

											<?php 
												if($this->session->logged_in)
														{
															if($this->session->logged_in->Id==$rs->uye_ID) { ?>
											<form action="<?=base_url()?>forum/oyunpaylasimsil/<?=$rs->Id?>" method="post">
                        ...
                        <div class="panel-body">
                            <div class="panel-group" id="accordion">
                             ...
                             <?php
								foreach($foyun as $rs)
								{								
								?>  <form onsubmit="return validateForm()" id="myForm"  action="<?=base_url()?>forum/oyunyorumkaydet/<?=$rs->Id?>" method="POST"> <?php } ?>
								...
								foreach($foyun as $rs)
								{								
								?> <form onsubmit="return validateForm1()" id="myForm1"  action="<?=base_url()?>forum/oyunyorumresimkaydet/<?=$rs->Id?>" method="post" enctype="multipart/form-data"><?php } ?>
