...
class Haberler extends CI_Controller {
		...
		$query=$this->db->get("haberler"); // haberler tablasonu veritananından çek
		$data["veri"]=$query->result(); // Sorgu sonucu gelen verileri data ya yükle
		
		
		$this->load->view('admin/header',$data);
		$this->load->view('admin/sidebar');
		$this->load->view('admin/haber/haberler_liste',$data);
		...
		{	
			...
			$this->load->view('admin/header',$data);
			$this->load->view('admin/sidebar');
			$this->load->view('admin/haber/haber_ekle');
			$this->load->view('admin/footer');
		...
		$sonuc=$this->main_Model->insert_data("haberler",$data);
		


		if($sonuc)
				{
					$this->session->set_flashdata("sonuc","Haber başarıyla eklendi.");
		...
		{
			$sonuc=$this->db->query('DELETE FROM haberler WHERE id='.$Id);
			$this->db->query('DELETE FROM haberleryorumlar WHERE haberId='.$Id);
			if($sonuc)
				{
					$this->session->set_flashdata("sonuc","Haber silme başarılıdır.");
					redirect(base_url()."admin/haberler");
				}else{
					$this->session->set_flashdata("sonuc","Yükleme sırasında hata oluştu !");
					redirect(base_url()."admin/haberler");
		...
		public function guncelle($Id)
		{
			...
			$sonuc=$this->main_Model->update_data("haberler",$data,$Id);
			if($sonuc)
				{
					$this->session->set_flashdata("sonuc","Haber güncelleme başarılıdır.");
					redirect(base_url()."admin/haberler");
				}else{
					$this->session->set_flashdata("sonuc","Yükleme sırasında hata oluştu !");
					redirect(base_url()."admin/haberler");
		...
		public function resimekle($Id)
		{
			...
			$this->load->view('admin/header',$data);
			$this->load->view('admin/sidebar');
			$this->load->view('admin/haber/haberler_resim',$data);
			$this->load->view('admin/footer');
		}
		...
		{		
				 ...
				 if ( ! $this->upload->do_upload('dosyaadi'))
                {
                        ...
                        $this->load->view('admin/haber/haberler_resim', $error);
                }
                else
                {
						...
						$this->main_Model->update_data("haberler",$data,$Id); //  fonksiypnua dataları id ile  gönder
